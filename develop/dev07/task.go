...
func or(channels ...<-chan interface{}) <-chan interface{} {
	...
	// Объединение каналов происходит при завершении ЛЮБОГО из них.
	// Если необходимо было дождаться завершения всех каналов,
	// необходимо вызывать wg.Add(1) в цикле.
	wg.Add(1)
	for _, c := range channels {
		go output(c)
	}

	// Ожидает завершения любого канала из waitgroup.
	wg.Wait()

	// Закрывает новый канал.
	close(out)
