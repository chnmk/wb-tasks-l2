...
/*
	Используется, когда поведение объекта должно изменяться в соответствии с его состоянием

	Плюсы:
		- Код, связанный с определенным состоянием, хранится в одном месте
		- Позволяет избавиться от множества условий типа if и switch

	Минусы:
		- Добавляет дополнительный уровень сложности к коду
		- Для добавления каждого нового состояния придется менять исходный объект

	Примеры использования:
		- Заказ, состояние которого меняется во время выполнения программы, и при обращении к нему в разных состояниях должны возвращаться разные данные
